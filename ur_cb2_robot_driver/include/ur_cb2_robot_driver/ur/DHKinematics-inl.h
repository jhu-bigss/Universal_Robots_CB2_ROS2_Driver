...
vctFrm4x4 DHKinematics<_NumJoints>::dhf(const size_type i, const double theta) const
{
  ...
  return RotZ(theta_[i] + theta) * TransXYZ(0, 0, d_[i]) * TransXYZ(a_[i], 0, 0) * RotX(alpha_[i]);
}
...
DHKinematics<_NumJoints>::geo_jac(const JointVector& q, const vct3 offset) const
{
  ...
  for (size_type i = 1; i <= kNUM_JOINTS; ++i)
  {
    ...
    // set angular jacobian column to z(i-1), i.e. the joint axis wrt to
    // base frame
    vctJacMatrixRef(jac, 3, i-1, 3, 1) =
        vctJacMatrixRef(T_i_to_0, 0, 2, 3, 1);  // pulls out the z-axis

    // calculate next link's reference frame
    T_i_to_0 = T_i_to_0 * dhf(i-1, q[i-1]);

    // set origin of link i
  ...
  // last origin must be translated by static offset
