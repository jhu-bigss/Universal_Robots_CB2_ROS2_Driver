...
#include "ur_robot_driver_cb2/ur_communication.h"
...
UrCommunication::UrCommunication(std::condition_variable& msg_cond,
	...
	if (pri_sockfd_ < 0) {
	RCLCPP_ERROR(rclcpp::get_logger("URPositionHardwareInterface"), "socket open failed");

	}
	sec_sockfd_ = socket(AF_INET, SOCK_STREAM, 0);
	if (sec_sockfd_ < 0) {
    RCLCPP_ERROR(rclcpp::get_logger("URPositionHardwareInterface"), "socket open failed 2");
	}
	server_ = gethostbyname(host.c_str());
	if (server_ == NULL) {
    RCLCPP_ERROR(rclcpp::get_logger("URPositionHardwareInterface"), "unknown host");
	}
...
bool UrCommunication::start() {
	...
	std::string cmd;
	bzero(buf, 512);
	RCLCPP_DEBUG(rclcpp::get_logger("URPositionHardwareInterface"), "connecting...");
	if (connect(pri_sockfd_, (struct sockaddr *) &pri_serv_addr_,
			sizeof(pri_serv_addr_)) < 0) {
    RCLCPP_DEBUG(rclcpp::get_logger("URPositionHardwareInterface"), "connecting error");
		return false;
	}
	RCLCPP_DEBUG(rclcpp::get_logger("URPositionHardwareInterface"), "connected");
	...
	std::this_thread::sleep_for(std::chrono::milliseconds(500));
  	...
  	RCLCPP_DEBUG(rclcpp::get_logger("URPositionHardwareInterface"), "Firmware detected : %.7f",robot_state_->getVersion());
	close(pri_sockfd_);

	RCLCPP_DEBUG(rclcpp::get_logger("URPositionHardwareInterface"), "Secondary interface connection");
	...
	if (flag_ < 0) {
	RCLCPP_DEBUG(rclcpp::get_logger("URPositionHardwareInterface"), "Secondary interface connection error");
		return false;
	}
		RCLCPP_DEBUG(rclcpp::get_logger("URPositionHardwareInterface"), "Secondary interface connected");
...
void UrCommunication::run() {
	...
	while (keepalive_) {
		...
		if (keepalive_) {
			//reconnect
				RCLCPP_DEBUG(rclcpp::get_logger("URPositionHardwareInterface"), "Secondary port connection error, trying again...");
			sec_sockfd_ = socket(AF_INET, SOCK_STREAM, 0);
			if (sec_sockfd_ < 0) {
        	RCLCPP_DEBUG(rclcpp::get_logger("URPositionHardwareInterface"), "Secondary interface connection error 2");
			}
			flag_ = 1;
			setsockopt(sec_sockfd_, IPPROTO_TCP, TCP_NODELAY, (char *) &flag_,
					sizeof(int));
			...
			while (keepalive_ && !connected_) {
				...
				getsockopt(sec_sockfd_, SOL_SOCKET, SO_ERROR, &flag_,
						&flag_len);
				if (flag_ < 0) {
						RCLCPP_ERROR(rclcpp::get_logger("URPositionHardwareInterface"), "Secondary interface connection to port 30002 failed");
				} else {
					connected_ = true;
						RCLCPP_DEBUG(rclcpp::get_logger("URPositionHardwareInterface"), "Secondary interface connection reconnected");
				}
			}
		}
	}
