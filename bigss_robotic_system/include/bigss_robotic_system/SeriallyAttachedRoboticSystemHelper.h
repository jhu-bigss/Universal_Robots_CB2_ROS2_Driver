...
#ifndef SERIALLYATTACHEDROBOTICSYSTEMHELPER_H
#define SERIALLYATTACHEDROBOTICSYSTEMHELPER_H
...
void PlaceMatIntoMatAtIndex(vctDoubleMat& input_matrix, vctDoubleMat& full_matrix, size_t index);
   ...
   */
inline void SlicePrmVelocityJointSet(const prmVelocityJointSet& full_command, prmVelocityJointSet& sliced_command, const int begin, const int sliced_size){
 ...
 * @param sliced_size length of the slice
 */
inline void SliceVctVec(const vctDoubleVec& full_vec, vctDoubleVec& sliced_vec, const int begin,  const int sliced_size){ 
 ...
 * @param sliced_size length of the slice
 */
inline void SliceVctVec(const vctBoolVec& full_vec, vctBoolVec& sliced_vec, const int begin,  const int sliced_size){
    std::copy(full_vec.begin()+begin, full_vec.begin()+begin+sliced_size, sliced_vec.begin());
}


inline void ConcatenateTwoVctVec(const vctVec& v1, const vctVec& v2, vctVec& v_out){
 ...
 */
inline vctDoubleMat AdjointJacobianByFrm3(const vctDoubleMat &jacobian, const vctFrm3 &transform){
...
inline vctDoubleMat ChangeFrameOfJacobian(const vctDoubleMat &jacobian, const vctFrm3 &transform){
    vctDoubleMat jac_out = jacobian;
...
inline vctDoubleMat HTPChangeBIGSSJacobianToOffsetFromEEF(const vctDoubleMat &jacobian, const vctFrm3 &transform, const vctFrm3 &self_fk){
    vctDoubleMat jac_out = jacobian;
 ...
 */
inline void Skew3VecTo3x3Mat(const vct3& vector, vctDouble3x3& mat){
mat.SetAll(0.0);
mat(2,1) = vector(0); //x
mat(0,2) = vector(1); //y
mat(1,0) = vector(2); //z
 ...
 */
inline void ConvertFrm3ToR6(const vctFrm3& frame, vctDoubleVec& r6_pos_axang){
 ...
 */
inline void ConvertR6ToFrm3(const vctDoubleVec& r6_pos_axang, vctFrm3& frame){
 ...
 */
inline vctFrm3 GetTransformFromTxt(const std::string &filename){
  vctFrm3 transform;
 ...
 */
inline void CalculateCartesianErrorFromR6(const vctDoubleVec &target_cp_R6, const vctDoubleVec &measured_cp_R6, vct3 &pos_err, vct3 &rot_ax_ang_err){
...
inline void PlaceMatIntoMatAtIndex(vctDoubleMat& input_matrix, vctDoubleMat& full_matrix, size_t index){
//TODO: size checks
...
#endif // SERIALLYATTACHEDROBOTICSYSTEMHELPER_H
